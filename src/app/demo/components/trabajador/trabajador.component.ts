...
export class TrabajadorComponent implements OnInit {
    ...
    this.trabajadorService.GetTrabajadores().subscribe({
      next: (data) => {
        this.trabajadorList = data.map((trabajador) => ({
          ...trabajador,
          apellidosynombres: `${trabajador.pla01apepaterno || ''} ${
            trabajador.pla01apematerno || ''
          } ${trabajador.pla01nombre1 || ''} ${
            trabajador.pla01nombre2 || ''
          }`.trim(),
        }));
      },
      error: (error) => {
  ...
  abrirDetalleTrabajador(
    ...
    if (trabajador) {
      ...
      const raw = {
        ...
        pla01nombre1: `${trabajador.pla01nombre1 || ''}${
          trabajador.pla01nombre2 ? ' ' + trabajador.pla01nombre2 : ''
        }`,
      ...
      const regimenArray = this.fb.array(
        (trabajador.regimenespensionarios ?? []).map((regimen) =>
          this.fb.group({
            ...
            pla31fechaini: [
              regimen.pla31fechaini ? new Date(regimen.pla31fechaini) : null,
            ],
            pla31fechafin: [
              regimen.pla31fechafin ? new Date(regimen.pla31fechafin) : null,
            ],
            pla31flagcomisionmixta: [
              regimen.pla31flagcomisionmixta === '1' ? 1 : 0,
            ],
          })
        )
      );
      ...
      const periodosArray = this.fb.array(
        (trabajador.periodoslaborales ?? []).map((periodo) =>
          this.fb.group({
            ...
            pla30fechaini: [
              periodo.pla30fechaini ? new Date(periodo.pla30fechaini) : null,
            ],
            pla30fechafin: [
              periodo.pla30fechafin ? new Date(periodo.pla30fechafin) : null,
            ],
  ...
  verTrabajador(trabajador: Trabajador): void {
    this.router.navigate(
      [
        `/home/maestros/trabajador/detalle-trabajador/${trabajador.pla01empresacod}/${trabajador.pla01empleadocod}`,
      ],
  ...
  guardarTrabajador() {
    if (this.trabajadorForm.valid) {
      ...
      const regimenespensionarios = raw.regimenespensionarios.map(
        (regimen: any) => ({
          ...regimen,
          pla31flagcomisionmixta:
            regimen.pla31flagcomisionmixta === 1 ? '1' : '0', // Convertir a cadena
        })
      );
        ...
        this.trabajadorService.CrearTrabajador(trabajadorEditado).subscribe({
          ...
          error: (err) => {
            verMensajeInformativo(
              ...
              'error',
              'Error',
              `No se pudo crear el trabajador: ${err.message}`
            );
          },
        });
      } else {
        this.trabajadorService
          .ActualizarTrabajador(trabajadorEditado)
          .subscribe({
            next: () => {
              verMensajeInformativo(
                this.messageService,
            ...
            error: (err) => {
              verMensajeInformativo(
                ...
                'error',
                'Error',
                `No se pudo actualizar el trabajador: ${err.message}`
              );
            },
          });
      }
  ...
  eliminarTrabajador(trabajador: Trabajador) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.trabajadorService
          .EliminarTrabajador(
            trabajador.pla01empresacod,
            trabajador.pla01empleadocod
          )
          .subscribe({
            next: () => {
              if (
            ...
            error: (err) => {
              verMensajeInformativo(
                ...
                'error',
                'Error',
                `No se pudo eliminar el trabajador: ${err.message}`
              );
            },
          });
      },
  ...
  regimenesLaborales: { codigo: string; descripcion: string }[] = [
    ...
    {
      codigo: '02',
      descripcion: 'PÚBLICO GENERAL -DECRETO LEGISLATIVO N°. 728',
    },
